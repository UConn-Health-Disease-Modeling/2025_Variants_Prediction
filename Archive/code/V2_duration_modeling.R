...
run_superlearner_binary_dual <- function(feat_data,
  ...
  X <- train_df %>% dplyr::select(-days_above_10, -days_above_10_cat)
  ...
  result_df1 <- model_df %>%
    dplyr::mutate(
      binary1_true = ifelse(Y1 == 1, "≥30", "<30"),
  ...
  result_df2 <- model_df %>%
    dplyr::mutate(
      binary2_true = ifelse(Y2 == 1, "100+", "<100"),
  ...
  final_df <- result_df1 %>%
    ...
    mutate(
      true_days_above_10_cat = model_df$days_above_10_cat,
...
heat_data <- country_acc %>%
  mutate(model = sub("^feat_\\d+_", "", model)) %>%  # 去掉feat_xx_前缀
...
ggplot(heat_data, aes(x = model, y = reorder(country, desc(country)), fill = accuracy)) +
  ...
  geom_text(aes(label = sprintf("%.3f", accuracy)), size = 3, color = "black") +
  scale_fill_gradientn(
    colors = c("#dceaf2", "#9dc3e6", "#6baed6", "#3182bd", "#08519c"), 
    limits = c(0.4, 1.0),
    oob = squish
  ) +
  labs(title = "Accuracy by country and model (21 Days Input)", fill = "Accuracy") +
