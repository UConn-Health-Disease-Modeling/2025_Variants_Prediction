...
# Australia        Austria         Brazil         Canada        Denmark         France        Germany          India          Italy 
#        83             36             71             80             56             84             64             67             77 
#     Japan    South Korea          Spain         Sweden United Kingdom  United States 
#        54             48             78             86             87             80 

# classified_data %>% select(country, lineage) %>% distinct() # 1051 combined variants 
...
peak_by_variant <- classified_data %>%
  ...
  filter(n() >= 35) %>%                                     # 1040 variants Kept 
...
legends_list_all <- measurements %>%
  dplyr::filter(days_above_10_cat == "100+", peak_share > 0.3) %>%
...
# )
# 
# saveRDS(all_data, file = "code2/all_data.rds")
