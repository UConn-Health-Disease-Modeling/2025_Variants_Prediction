...
fit_and_oof_nnet <- function(sp, seed = 1L) {
  ...
  best_params <- nn_fit$bestTune
  
  oof <- nn_fit$pred
  oof <- oof[oof$size == best_params$size & abs(oof$decay - best_params$decay) < .Machine$double.eps^0.5, , drop = FALSE]
  ...
  list(
    ...
    varimp            = tryCatch(caret::varImp(nn_fit), error = function(e) NULL)
  )
}

results1 <- lapply(splits1, fit_and_oof_nnet, seed = proc_number)
results2 <- lapply(splits2, fit_and_oof_nnet, seed = proc_number)

results <- list(results1 = results1, results2 = results2)

dir.create("../../result/Share_30/nnet", showWarnings = FALSE, recursive = TRUE)
outfile <- file.path("../../result/Share_30/nnet", sprintf("nnet_results_seed%03d.rds", proc_number))
