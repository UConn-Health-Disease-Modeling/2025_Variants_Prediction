...
fit_and_oof_mars <- function(sp, seed = 1L) {
  ...
  max_prune <- min(100L, max(10L, ncol(X) * 2L))
  grid <- expand.grid(
    degree  = c(1, 2, 3),
    nprune  = seq(5L, max_prune, by = 5L)
  )
  ...
  best_params <- mars_fit$bestTune
  
  oof <- mars_fit$pred
  oof <- oof[oof$degree == best_params$degree & oof$nprune == best_params$nprune, , drop = FALSE]
  ...
  list(
    ...
    varimp            = caret::varImp(mars_fit)
  )
}

results1 <- lapply(splits1, fit_and_oof_mars, seed = proc_number)
results2 <- lapply(splits2, fit_and_oof_mars, seed = proc_number)

results <- list(results1 = results1, results2 = results2)

dir.create("../../result/Share_30/mars", showWarnings = FALSE, recursive = TRUE)
outfile <- file.path("../../result/Share_30/mars", sprintf("mars_results_seed%03d.rds", proc_number))
