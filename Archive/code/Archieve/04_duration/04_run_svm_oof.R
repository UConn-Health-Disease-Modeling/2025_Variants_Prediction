...
suppressPackageStartupMessages({
  library(dplyr)
  library(caret)
  library(kernlab)  # for svm (svmRadial in caret)
})
...
fit_and_oof_svm <- function(sp, seed = 1L) {
  ...
  grid <- expand.grid(
    sigma = 10^seq(-3, 1, length.out = 8),
    C     = 10^seq(-2, 2, length.out = 8)
  )
  ...
  oof <- svm_fit$pred
  oof <- oof[abs(oof$sigma - best_params$sigma) < .Machine$double.eps^0.5 &
               abs(oof$C - best_params$C) < .Machine$double.eps^0.5, , drop = FALSE]
  ...
  list(
    ...
    varimp            = tryCatch(caret::varImp(svm_fit), error = function(e) NULL)
  )
}

results1 <- lapply(splits1, fit_and_oof_svm, seed = proc_number)
results2 <- lapply(splits2, fit_and_oof_svm, seed = proc_number)

results <- list(results1 = results1, results2 = results2)

dir.create("../../result/Share_30/svm", showWarnings = FALSE, recursive = TRUE)
outfile <- file.path("../../result/Share_30/svm", sprintf("svm_results_seed%03d.rds", proc_number))
