...
fit_and_oof_classif <- function(sp, seed = 1L) {
  ...
  p <- ncol(X)
  mtry_vals <- sort(unique(pmax(1L, round(c(sqrt(p), p/4, p/2)))))
  ...
  oof <- rf_fit$pred
  oof <- oof[oof$mtry == best_params$mtry, , drop = FALSE]
...
dir.create("../../result/Share_30/rf", showWarnings = FALSE, recursive = TRUE)
outfile <- file.path("../../result/Share_30/rf", sprintf("rf_results_seed%03d.rds", proc_number))
