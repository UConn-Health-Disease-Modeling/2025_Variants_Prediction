...
suppressPackageStartupMessages({
  library(dplyr)
  library(caret)
  library(xgboost)   # used through caret::train(method = "xgbTree")
})
...
fit_and_oof_xgb <- function(sp, seed = 1L) {
  ...
  for (nm in names(best_params)) {
    oof <- oof[oof[[nm]] == best_params[[nm]], , drop = FALSE]
  }
  ...
  list(
    ...
    varimp            = caret::varImp(xgb_fit)
  )
}

results1 <- lapply(splits1, fit_and_oof_xgb, seed = proc_number)
results2 <- lapply(splits2, fit_and_oof_xgb, seed = proc_number)

results <- list(results1 = results1, results2 = results2)

dir.create("../../result/Share_30/xgb", showWarnings = FALSE, recursive = TRUE)
outfile <- file.path("../../result/Share_30/xgb", sprintf("xgb_results_seed%03d.rds", proc_number))
