...
source('Collab/functions/functions_0417.R')

url<-"C://Users//young//OneDrive//Desktop//UKHSA_COVID19_Variants//summary_gisaid.csv"
...
# gives you the alias for any lineage
source(glue::glue('C://Users//young//OneDrive//Desktop//UKHSA_COVID19_Variants//Original//functions_0417.R'))

alias_list <- get_alias()

this_alias <- dplyr::filter(alias_list, alias == 'JN') |>
  dplyr::pull(alias_lineage); this_alias
# BQ prefix = BA.5.3.1.1.1.1; this means BQ.1 is BA.5.3.1.1.1.1.1, and any sub lineage
...
summarised <- with_alias |>
  dplyr::mutate(
    is_target_lineage = stringr::str_starts(unaliased_lineage, paste0(this_alias, '')) | unaliased_lineage == paste0(this_alias, '')
  ) |>
  dplyr::summarise(
...
panel_A <- output |> 
  dplyr::filter(date <= lubridate::ymd('20240130')) |>
  ggplot2::ggplot(
...
ggplot2::ggsave(panel_A + panel_B, file = glue::glue('C://Users//young//OneDrive//Desktop//UKHSA_COVID19_Variants//outputs//comparison_figure_{lineage}.png'), dpi = 300, width = 12, height = 6)
