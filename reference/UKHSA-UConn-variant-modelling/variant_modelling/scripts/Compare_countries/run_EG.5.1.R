...
# Test run - compare EG.5 lineages for SK and UK 
lineage <- 'EG.5.1'
...
this_alias <- dplyr::filter(alias_list, alias == 'EG') |>
  dplyr::pull(alias_lineage); this_alias
# EG prefix = XBB.1.9.2; this means EG.5.1 is XBB.1.9.2.5.1, and any sub lineage
...
summarised <- with_alias |>
  dplyr::mutate(
    is_target_lineage = stringr::str_starts(unaliased_lineage, paste0(this_alias, '.5.1.')) | unaliased_lineage == paste0(this_alias, '.5.1')
  ) |>
  dplyr::summarise(
