...
this_alias <- dplyr::filter(alias_list, alias == 'BQ') |>
...
summarised <- with_alias |>
  dplyr::mutate(
    is_target_lineage = stringr::str_starts(unaliased_lineage, paste0(this_alias, '.1.')) | unaliased_lineage == paste0(this_alias, '.1')
  ) |>
  dplyr::summarise(
...
panel_A <- output |> 
  dplyr::filter(date <= lubridate::ymd('20230801')) |>
  ggplot2::ggplot(
