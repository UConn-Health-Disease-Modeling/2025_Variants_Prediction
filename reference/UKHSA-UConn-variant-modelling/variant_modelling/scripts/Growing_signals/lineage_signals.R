...
dataset <- readr::read_csv(glue::glue('{folder_wd}/variant_modelling/data/summary_gisaid.csv')) |>
  dplyr::filter(country == country_select,
                lineage != 'Unassigned',
                date >= max(date) - months_from_most_recent) |>
  dplyr::mutate(
    alias = gsub(
      pattern = "[^a-zA-Z]",
      replacement = "",
...
p_lim <- 0.015
# How many sequenced cases do we require before we model a limeage group
n_lim <- 50
...
p1 <- output |>
  ...
  ggplot2::labs(title = glue::glue('{country_select} lineage share
...
p2 <- output |>
  ...
  ggplot2::labs(title = glue::glue('{country_select} lineage growth rates
...
ggplot2::ggsave(combined_plot, file = glue::glue('{folder_wd}/variant_modelling/outputs/{country_select}_lineages_{unique(final_date$date)}.png'),
